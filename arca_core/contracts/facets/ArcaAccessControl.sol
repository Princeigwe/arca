...
contract ArcaAccessControl {
		function arcaIdentityRegistryFacetRegisterMedicalGuardian(
				...
				(bool success, ) = address(this).call(
						abi.encodeWithSignature(
								"registerMedicalGuardian(address,uint256,address)",
								_guardianAddress,
								_addedAt,
								_addedBy
						)
				...
				if (!success) {
						revert(
								"Something went wrong with registering medical guardian to identity registry facet"
						);
				}
		}
		...
		function verifyAccessToPatientIdentityData(
				address _requester,
				address _mainPatientAddress
		) public view returns (bool) {
				LibADS.DiamondStorage storage ds = LibADS.diamondStorage();
				require(
						ds.patientExists[_mainPatientAddress],
						LibADS.PatientDoesNotExistError(_mainPatientAddress)
				);
		...
		/// @param _canGrantMedicalProviderAccess permission.
		/// @param _canGrantMedicalGuardianAccess permission.
		/// @param _canRevokeMedicalProviderAccess permission.
		/// @param _canRevokeMedicalGuardianAccess permission.
		...
		function assignMedicalGuardian(
				...
				bool _canGrantMedicalProviderAccess,
				bool _canGrantMedicalGuardianAccess,
				bool _canRevokeMedicalProviderAccess,
				bool _canRevokeMedicalGuardianAccess,
				...
				require(
						...
						LibADS.AuthorizationError(
								"Error assigning medical guardian: Medical guardian entity does not exist for this sender"
						)
				);
				require(
						ds.isMedicalGuardianOfPatient[msg.sender][_mainPatientAddress],
						LibADS.AuthorizationError(
								"Error assigning medical guardian: Sender is not a medical guardian to the patient"
						)
				);
				require(
						ds
						.medicalGuardianPermissionsOnPatient[msg.sender][
								_mainPatientAddress
						].role == LibADS.MedicalGuardianRole.PRIMARY,
						LibADS.AuthorizationError(
								"Error assigning medical guardian: Sender is not a primary medical guardian to the patient"
						)
				);
				if (
						_role != LibADS.MedicalGuardianRole.PRIMARY &&
						(_canGrantMedicalProviderAccess ||
								_canGrantMedicalGuardianAccess ||
								_canRevokeMedicalProviderAccess ||
								_canRevokeMedicalGuardianAccess ||
								_canUploadRecords ||
								_canDeleteRecords)
						// _canReadRecords  // this permission is allowed for a secondary medical guardian
				) {
						revert LibADS.AuthorizationError(
								"Error assigning medical guardian to patient: Secondary guardian is not allowed to perform delicate action(s)"
						);
				}
				...
				if (!ds.medicalGuardianExists[_medicalGuardian]) {
						arcaIdentityRegistryFacetRegisterMedicalGuardian(
								_medicalGuardian,
								block.timestamp,
								msg.sender
						);
				...
				ds.isMedicalGuardianOfPatient[_medicalGuardian][
						_mainPatientAddress
				] = true;
				ds.patientMedicalGuardians[_mainPatientAddress].push(
						ds.medicalGuardianAccount[_medicalGuardian]
				);
				ds.medicalGuardianPermissionsOnPatient[_medicalGuardian][
						_mainPatientAddress
				] = LibADS.MedicalGuardianPermission({
						role: _role,
						guardian: _medicalGuardian,
						patient: _mainPatientAddress,
						canGrantMedicalProviderAccess: _canGrantMedicalProviderAccess,
						canGrantMedicalGuardianAccess: _canGrantMedicalGuardianAccess,
						canRevokeMedicalProviderAccess: _canRevokeMedicalProviderAccess,
						canRevokeMedicalGuardianAccess: _canRevokeMedicalGuardianAccess,
						canUploadRecords: _canUploadRecords,
						canReadRecords: _canReadRecords,
						canDeleteRecords: _canDeleteRecords
				});

				ds.medicalGuardianPermissions[_medicalGuardian].push(
						ds.medicalGuardianPermissionsOnPatient[_medicalGuardian][
								_mainPatientAddress
						]
				);
				...
				ds.patientAccount[_mainPatientAddress].rsaMasterDEKs.push(
						LibADS.IdentityRSAMasterDEK({
				...
				ds.patientAddressCid[_mainPatientAddress] = _cid;
		...
		function updateMedicalGuardianPermission(
				...
				require(
						ds.medicalGuardianExists[msg.sender],
						LibADS.AuthorizationError(
								"Error updating medical guardian permissions: Medical guardian entity does not exist for this sender"
						)
				);
				require(
						ds.isMedicalGuardianOfPatient[msg.sender][_mainPatientAddress],
						LibADS.AuthorizationError(
								"Error updating medical guardian permissions: Sender is not a medical guardian to the patient"
						)
				);
				require(
						ds
						...
						LibADS.AuthorizationError(
								"Error updating medical guardian permissions: Sender is not a primary medical guardian to the patient"
						)
				);
				...
				if (
						...
						revert LibADS.AuthorizationError(
								"Error updating medical guardian permissions: Secondary guardian is not allowed to perform delicate action(s)"
						);
				}
				...
				ds.medicalGuardianPermissionsOnPatient[_medicalGuardian][
						...
						guardian: _medicalGuardian,
						patient: _mainPatientAddress,
						canGrantMedicalProviderAccess: _canGrantProviderAccess,
						canGrantMedicalGuardianAccess: _canGrantGuardianAccess,
						canRevokeMedicalProviderAccess: _canRevokeProviderAccess,
						canRevokeMedicalGuardianAccess: _canRevokeGuardianAccess,
						canUploadRecords: _canUploadRecords,
						canReadRecords: _canReadRecords,
						canDeleteRecords: _canDeleteRecords
				});
		...
		function revokeMedicalGuardianPermission(
				address _medicalGuardian,
				address _mainPatientAddress,
				bytes memory _cid
		) public {
				...
				LibADS.MedicalGuardian[] storage patientMedicalGuardians = ds
						.patientMedicalGuardians[_mainPatientAddress];
				uint256 medicalGuardianIndex;
				bool medicalGuardianFound = false;

				for (uint256 i = 0; i < patientMedicalGuardians.length; i++) {
						if (
								patientMedicalGuardians[i].guardianAddress == _medicalGuardian
						) {
				...
				require(
						medicalGuardianFound,
						LibADS.MedicalGuardianDoesNotExistError(_medicalGuardian)
				);
				// removing the medical guardian from the patient's medical guardians array
				patientMedicalGuardians[medicalGuardianIndex] = patientMedicalGuardians[
						patientMedicalGuardians.length - 1
				];
				patientMedicalGuardians.pop();

				// locating the permission to be removed
				LibADS.MedicalGuardianPermission[]
						storage medicalGuardianPermissions = ds.medicalGuardianPermissions[
								_medicalGuardian
						];
				...
				require(
						medicalGuardianPermissionFound,
						LibADS.MedicalGuardianPermissionDoesNotExistError(_medicalGuardian)
				);
				// removing the permission from the medical guardian's permissions array
				medicalGuardianPermissions[
						medicalGuardianPermissionIndex
				] = medicalGuardianPermissions[medicalGuardianPermissions.length - 1];
				medicalGuardianPermissions.pop();

				// locating the RSA master DEK to be removed
				LibADS.IdentityRSAMasterDEK[] storage rsaMasterDEKs = ds
						.patientAccount[_mainPatientAddress]
						.rsaMasterDEKs;
				...
				require(
						medicalGuardianDekFound,
						"Error revoking medical guardian permission: RSA master DEK for medical guardian not found"
				);
				// removing the RSA master DEK from the patient's account
				rsaMasterDEKs[medicalGuardianDekIndex] = rsaMasterDEKs[
						rsaMasterDEKs.length - 1
				];
				rsaMasterDEKs.pop();
				...
				delete ds.medicalGuardianPermissionsOnPatient[_medicalGuardian][
						_mainPatientAddress
				];
		...
		function getMedicalPermission(
				...
				view
				returns (
						LibADS.MedicalGuardianPermission memory _medicalGuardianPermission
				)
		{
				LibADS.DiamondStorage storage ds = LibADS.diamondStorage();
				require(
						ds.patientExists[_patient],
						LibADS.PatientDoesNotExistError(_patient)
				);
				require(
						ds.isMedicalGuardianOfPatient[_medicalGuardian][_patient],
						LibADS.AuthorizationError(
								"Error getting medical permission: Sender is not a medical guardian to patient"
						)
				);
				_medicalGuardianPermission = ds.medicalGuardianPermissionsOnPatient[
						_medicalGuardian
				][_patient];
		}

		/// @notice This function is used for a medical guardian(current sender) to see all permissions they have.
		function getMyMedicalGuardianPermissions()
				public
				view
				returns (
						LibADS.MedicalGuardianPermission[]
								memory _medicalGuardianPermissions
				)
		{
				LibADS.DiamondStorage storage ds = LibADS.diamondStorage();
				require(
						ds.medicalGuardianExists[msg.sender],
						LibADS.AuthorizationError(
								"Error getting current medical guardian permissions: Medical guardian entity does not exist for this sender"
						)
				);
